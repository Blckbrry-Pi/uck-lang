    ...
    if let Some(LexerToken::Identifier(name)) = lxr.next() {
        ...
        if let Some(LexerToken::Colon) = lxr.peek() {
            ...
            match lxr.next() {
                ...
                Some(LexerToken::Identifier(aliased_name)) => Ok(
                    AstDestructuringPattern::AliasedName(
                ...
                Some(LexerToken::LeftCurlyBrace) => {
                    ...
                    loop {
                        ...
                        match lxr.peek() {
                            Some(LexerToken::Identifier(_)) => child_destructuring_patterns.push(parse_destructuring_pattern(lxr)?),
                            ...
                            Some(_) => {
                                ...
                                return Err(ParseError::unexpected_token_error(lxr.slice().unwrap(), span, EXPECTED_ARR, true));
                            },
                            None => {
                                lxr.next();
                                return Err(ParseError::end_of_file_error(&["identifier or `}` (as part of a destructuring pattern)"], true));
                            }
                        }
                        ...
                        match lxr.next() {
                            ...
                            Some(_) => {
                                let span = lxr.span().unwrap();
                                return Err(
                                    ParseError::unexpected_token_error(lxr.slice().unwrap(), span, EXPECTED_ARR_2, true)
                                );
                            },
                            None => return Err(ParseError::end_of_file_error(EXPECTED_ARR_2, true)),
                        }
                    }

                    Ok(
                        AstDestructuringPattern::Destructured(
                            base_span.start..lxr.span().unwrap().end,
                            name,
                            Box::new(child_destructuring_patterns),
                        )
                    )
                },
                ...
                // If this matches the end of the file matches `[name]: [EOF]`
                None => Err(ParseError::end_of_file_error(&["identifier or `{` (as part of a destructuring pattern)"], true))
            }
        }
